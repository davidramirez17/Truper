...
import { Activity, AlertCircle, ArrowDownToLine, ArrowRight, ArrowUpRight, BarChart3, Check, CheckCircle2, ChevronLeft, ChevronRight, Database, FileSpreadsheet, FolderKanban, HardDrive, Info, Moon, Plus, Search, ShieldCheck, Sun, TrendingUp, Users, Wallet } from 'lucide-react';
...
function Metrics({ analytics }: Pick<Props, 'analytics' | 'data'>) {
  const hasData = analytics.current.length > 0;
  const cards = [
    { label: 'Ventas del periodo', value: hasData ? money(analytics.total) : '—', icon: Wallet, detail: 'Importe declarado en el archivo', primary: true },
  ...
  return <section className="metrics-grid" aria-label="Indicadores del periodo">{cards.map(card => <MetricCard key={card.label} {...card} detail={hasData ? card.detail : 'Sin registros en el periodo'} change={card.primary ? analytics.change : undefined} />)}</section>;
}
...
export function ProjectsView({ data, href, onUpload, onCreate }: Props) {
...
export function SettingsView({ data, theme, onTheme }: { data: WorkspaceData; theme: string; onTheme: (theme: string) => void }) {
  return <div className="settings-grid"><section className="panel settings-card"><PanelTitle title="Apariencia" subtitle="La misma experiencia en cada módulo." /><div className="theme-options">{[{ id: 'light', label: 'Claro', icon: Sun }, { id: 'dark', label: 'Oscuro', icon: Moon }].map(option => <button key={option.id} className={theme === option.id ? 'selected' : ''} aria-pressed={theme === option.id} onClick={() => onTheme(option.id)}><span className={`theme-preview preview-${option.id}`}><i /><b /><b /><b /></span><span><option.icon size={16} />{option.label}{theme === option.id && <Check size={16} />}</span></button>)}</div><Link href="/diseno" className="text-button source-connection-link">Explorar el sistema de diseño <ArrowUpRight size={15} /></Link></section><section className="panel settings-card"><PanelTitle title="Mi cuenta" subtitle="Tu identidad y tu acceso." /><div className="setting-row"><span><Users size={22} /></span><div><strong>{data.profile?.full_name || 'Mi cuenta'}</strong><p>{data.profile?.email}</p><p>Tu rol y los accesos se administran desde Usuarios.</p></div></div><div className="account-actions"><Link href="/recuperar" className="button secondary">Cambiar contraseña</Link><form action="/auth/signout" method="post"><button className="button secondary">Cerrar sesión</button></form></div></section><section className="panel roadmap-card"><PanelTitle title="Una base preparada para lo que sigue" subtitle="Módulos independientes, un mismo sistema de diseño." /><div className="roadmap">{[{ step: '01', title: 'Cuentas y permisos', text: 'Sesiones verificadas, aprobación de usuarios y acceso por proyecto.', ready: true }, { step: '02', title: 'Archivos que se convierten en información', text: 'Relación de columnas, validación, carga atómica e historial en Supabase.', ready: true }, { step: '03', title: 'Diseño para todos los procesos', text: 'Componentes compartidos de indicadores, tablas, estados, navegación y tema.', ready: true }, { step: '04', title: 'Más procesos y nuevas fuentes', text: 'Inventario, cobranza y pedidos tendrán sus propios contratos cuando se revisen sus archivos. Después: conectores y sincronización.', ready: false }].map(step => <div className="roadmap-step" key={step.step}><span className={step.ready ? 'step-complete' : ''}>{step.ready ? <Check size={18} /> : step.step}</span><div><h3>{step.title}</h3><p>{step.text}</p></div><span className={`badge ${step.ready ? 'badge-ready' : 'badge-planned'}`}>{step.ready ? 'Implementado' : 'Siguiente etapa'}</span></div>)}</div></section><div className="notice info settings-note"><ShieldCheck size={20} /><p>Los registros guardados pertenecen a tus proyectos. Los archivos originales no se conservan: guarda una copia de cada Excel de origen.</p></div></div>;
}
